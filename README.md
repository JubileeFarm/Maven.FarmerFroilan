...
* Consider a system in which
	* `com.zipcodewilmington.froilansfarm.Eater` can `eat` an `Edible` object.
	* `NoiseMaker` can `makeNoise`
	* `Animal` is a `NoiseMaker` and `com.zipcodewilmington.froilansfarm.Eater`
	...
	* `Farmer` is an `com.zipcodewilmington.froilansfarm.Eater`, a `Rider`, a `Botanist`, and a `Person`.
